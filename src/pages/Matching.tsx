...
import { Loader2, Users, Home, ChevronDown, ChevronLeft, RefreshCw, AlertTriangle, Trash2 } from 'lucide-react';
  ...
  return (
    <PageContainer>
      <PageHeader
        ...
        description={`${matchesCount} ${matchesCount === 1 ? 'match' : 'matches'} • ${buyersCount} ${buyersCount === 1 ? 'buyer' : 'buyers'} • ${propertiesCount} ${propertiesCount === 1 ? 'property' : 'properties'} • Last synced: ${formatLastSynced(lastSynced)}`}
        actions={
          <div className="flex items-center gap-2">
            {isStale && (
              <div className="flex items-center gap-1 text-sm text-amber-600">
                <AlertTriangle className="h-4 w-4 text-amber-500" />
                <span>Cache outdated</span>
              </div>
            )}
            <Button
